...
save_csv_from_sheet <- function(){
  sheet_map = data.frame(row.names = c(2,3,4,5),
                         val=c("projects", "cameras", "deployments", "images"))
  ...
  for(sheet in 2:5){
    ...
    write_csv(df,  here(path_templates, paste(batch_type, ".csv", sep="")))
    ...
    write_csv(df, here(path_templates, paste(batch_type, "_required.csv", sep="")))
  }
}
# other stuff ----
wi_batch_type = "images"
df_length = 1
wi_batch_function <- function(wi_batch_type, df_length){
  csv <- read_csv(here(path_templates, paste(wi_batch_type, ".csv", sep="")))
...
wi_batch_function_req <- function(wi_batch_type, df_length){
  csv = read_csv(here(path_templates, paste(wi_batch_type, "_required.csv", sep="")))
  colnames = colnames(csv)
  df = data.frame(matrix(ncol = length(colnames),nrow=df_length))
  colnames(df) <- colnames 
  return(df)
...
prep_upload <- function(img_bu, cam_bu, dep_bu, prj_bu){
  ...
  write_csv(img_bu,here(path_templates, paste(folder_name, "/images.csv", sep="")), na ="")
  write_csv(dep_bu,here(path_templates, paste(folder_name, "/deployments.csv", sep="")), na ="")
  write_csv(prj_bu,here(path_templates, paste(folder_name, "/projects.csv", sep="")), na ="")
  write_csv(cam_bu,here(path_templates, paste(folder_name, "/cameras.csv", sep="")), na ="")
...
prep_split_upload <- function(img_bu, cam_bu, dep_bu, prj_bu, parts){
  ...
  for(dep_sp in dep_bu_sps){
    ...
    dir.create(folder_name, showWarnings = FALSE)
    write_csv(img_bu_split,here(path_templates, paste(folder_name,"/images.csv", sep="")), na ="")
    write_csv(dep_sp,here(path_templates, paste(folder_name, "/deployments.csv", sep="")), na ="")
    write_csv(prj_bu,here(path_templates, paste(folder_name,"/projects.csv", sep="")), na ="")
    write_csv(cam_bu,here(path_templates, paste(folder_name,"/cameras.csv",sep="")), na ="")
    i = i + 1
